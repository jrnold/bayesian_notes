...
Both **complete separation** and **quasi-complete** separtion cause problems for binomial maximum likelihood estimators.

## Example: Complete Separation Data

The following data is an example of data with complete separation.[^fake-separation]
```{r}
jrnold.bayes.notes::QuasiSeparation
```
...
```{r}
library("ggplot2")
ggplot(complete_sep, aes(x = x, y = y)) + 
  geom_point()
```
...
```{r}
jrnold.bayes.notes::CompleteSeparation
```
