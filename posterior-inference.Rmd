...
alexseev <- mutate(alexseev,
                   brdcont = as.integer(brdcont))
...
slav_x_change %>%
  ...
  summarise(q2.5 = quantile(dydx, 0.025),
            ...
            mean = mean(dydx)) %>%
  ggplot(aes(x = mean, y = mean, ymin = q2.5, ymax = q97.5)) +
...
rec <- recipe(bushvote ~ retecon + white + female + age + educ1_7, income + 
              partyid + bushiraq,
       data = votechoice) %>%
  step_dummy(retecon) %>%
  step_dummy(educ1_7) %>%
