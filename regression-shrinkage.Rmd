...
A utility function to extract the number of non-zero coefficients for lasso with the penalty chosen by cross-validation.
```{r}
...
```{r}
tau0 <- 5 / (11 - 5)  / sqrt(length(diabetes$y))
...
```{r}
models <- list()
for (i in c("lm_shrinkage_student_t_1", 
...
```{r}
glmnet(diabetes_standata$X, diabetes_standata$y) %>%
  tidy()
```
...
fit_hs <- sampling(models[["lm_shrinkage_hs"]], data = diabetes_standata_hs, control = list(adapt_delta = 0.99))
```

```{r}
diabetes_standata_laplace <- within(diabetes_standata, {
  df_tau <- 1
  scale_tau <- 2.5
  df_lambda <- 4
})
fit_laplace <- sampling(models[["lm_shrinkage_laplace_1"]], data = diabetes_standata_laplace, control = list(adapt_delta = 0.95))
```
...
fit_normal <- sampling(models[["lm_shrinkage_normal_1"]], data = diabetes_standata_normal, control = list(adapt_delta = 0.95))
```
...
fit_student <- sampling(models[["lm_shrinkage_student_t_1"]], data = diabetes_standata_student_t, control = list(adapt_delta = 0.99))
```
...
-  Compare performance of sampling
-  Out of sample predictions - using LOO
