...
Also see the `r rpkg("bayesplot")` vignette [Visual MCMC diagnostics using the bayesplot package](https://cran.r-project.org/web/packages/bayesplot/vignettes/MCMC-diagnostics.html), which though specific to the provides, provides a good overview of these diagnostics.
...
```{r fig.caption = sprintf("Effective sample size for time series of %d and varying autocorrelation", n)}
...
Trace plots are a timeseries of sampler iterations, e.g. as produced by `r rdoc("bayesplot", "mcmc_trace")`.
...
See @FlegalHaranJones2008a and the `r rpkg("mcmcse")` for methods to calculate MCMC standard errors for means and quantiles using subsampling methods. 
