...
@VehtariGelmanGabry2017a propose a method to regularize the importance weights called Pareto-Smoothed Importance Sampling (PSIS-LOO).
...
Observations where the estimated shape parameter of that Pareto distribution is $\hat{k} > 0.7$ are unstable, and the PSIS-LOO approximation is poor [@VehtariGelmanGabry2017a].
