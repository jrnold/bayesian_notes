...
2.  Functional: find a function that approximates the true posterior. e.g. maximum a posteriori, Laplace/quadratic approximation. 
2.  Grid/Quadrature. approximate the posterior with a discrete distribution evaluated at a fixed set of points.
3.  Sampling. draw a sample from the posterior.
...
- Fitting a mixture of normal densitities. This is especially useful for multi-modal densities. [@BDA3, p. 319]
...
```{r}
ret <- optim(0.5, fn = fpost, hessian = TRUE, method = "Brent",
             lower = 0, upper = 1)
```
...
Deterministic numerican intergration is based on evaluating $h(\theta) p(\theta | y)$ at a set of points $\theta^1, \dots, \theta^S$.
$$
...
2. Let $x = P^{-1}(u)$, where $P^{-1}(X)$ is the inverse of the CDF (quantile) function for $X$.
...
2. We would have to CDF of the posterior distribution analytically, but in hard problems that is something we don't know.
...
$$
This will also work with an unnormalized density function.
...
tl;dr: Sample from a proposal density and reject with a propability proportional to the ratio of the target density to the proposal density. Works best if the proposal density is close to the target density.

Rejection sampling consists of the following:

1. Sample from a proposal density, e.g. the prior $p(\theta)$
2. Accept with probability $p(\theta | y) / p(\theta)$
...
2.  With probability $p(\theta | y) / M g(\theta)$ accept $\theta$ as a draw from $p$. If rejected, return to 1.
...
tl;dr: Draw a sample from a target distribution from an approximate distribution weight those samples by the ratio of target distributon to the approximate distribution.  Works best if the proposal density is close to the target density, or at least wider than the target density.
...
-   Sequential Monte Carlo (SMC) is a variant that is particularly useful for updating posterior distributions when the data arrives sequentially---either real time or because it involves time series data [@CarvalhoEtAl2010a].
-   To update a model when a similar posterior is available. E.g. the posterior distribution for leave-one-out cross validation [@VehtariGelmanGabry2015a]. The distribution $p(\theta | y_{-i})$ is likely to be similar to $p(\theta | y)$, so if we have already calculated $p(\theta | y)$ we can use the later as the proposal distribution for $p(\theta | y)$.

See @BDA3 [Sec 10.4], @GelfandSmith1990a, @LopesPolsonCarvalho2012a, and @SmithGelfand1992a for more on importance sampling.
...
1.  Draw a starting value $\theta^0$ for which $p(\theta^0 | y) > 0$. This can be specified manually or sampled from a crude approximation.
2.  For iterations *t = 1, 2, ...$:

    1.  Sample a *proposal* value $\theta^*$ from a *jumping distribution* (*proposal distribution*) at time $t$, $J_t(\theta^* | \theta^{t - 1})$.
    2.  Calculate the ratio of the density of the proposal and target distributions,
        $$
...
2.  jumping distribution $J_t(\theta^t | \theta^{t - 1}),
...
2.  Be able to calculate the density ratio $r$, usually meaning that both $p(\theta^* | y)$ and $p(\theta^{t - 1} | y)$ must be able to be calculated for all $\theta$.
...
-   For this to work the Markov chain this produces must have stationary distribution that [@BDA3, p. 279].
...
-   MH is similar to a dynamic form of rejection sampling or importance sampling. 
    However, whereas those methods have a static proposal distribution, the jumping distribution, while fixed in its parameters, of the HM algorithm is recentered around the last draw, so it can move to areas of higher density.
...
-   For iterations, $t = 1, \dots$:
    ...
    -   For $j \in 1, \dots, d$
        ...
        -   Draw a value from the conditional distribution of $\theta_j$ given all the other parameters,
            ...
            $$
            where $\theta^{t - 1}_{-j}$ has consists of updated parameters for all parameters preceeding $j$ and the previous iteration's values for all succeeding parameters,
            $$
...
1.  The iterations have not proceeded long enough to find the typical set and are not drawing samples from the target distribtion.
2.  The samples have within-sequence correlations. Serial correlation is not a fatal problem. Within 

How to monitor and assess iterative simulation

1.  Design simulations to allow monitoring of convergence by running multiple chains from dispersed starting points.
2.  Monitor convergence by comparing within chain variation and between variation using the $\hat{R}$ statistic. Only when the between and within variations are approximately equal will the multiple chains appear to be sampling from the same distribution.
3.  Require a certain level of efficiency in the output in terms of acceptable sample size.
    If that that efficiency is too costly to obtain, it may require tweaking the model or the algorithm to be more efficient. See @BDA [Ch. 12] for methods to speed up MCMC.
